...
/********** Internal Signal **********/
	...
	assign bp_delot_en = (ex_bp_error)?`DISABLE:
								(bp_dir == 4'b1000 && bp_en == 1)? `ENABLE:`DISABLE;
	assign bp_delot_pc = (ex_bp_error)?`WORD_ADDR_W'b0:
...
module RAS_LIFO(
	...
	/**** Ras flush ****/
	input  wire bp_error,
	input  wire [`WordAddrBus] error_pc,
	/**** Addr_Predictor Signal ****/
	input  wire ras_write_en,
	input  wire [`WordAddrBus] call_pc,
	...
	reg 	[`RasAddrBus]	ras_pointer;
	reg 	[`RepeatCounterBus] repeat_counter;
	reg 	[`WordAddrBus]	last_pc;
	...
	assign ras_read_data = (ras_read_en & ras_write_en) ? ras_write_data:
							(ras_read_en) ? ras_data[ras_pointer] : `WORD_DATA_W'b0;
	assign repeat_taken = |repeat_counter;
	//Write Channel
	assign repeat_en = (call_pc == last_pc);
	assign ras_flush = (bp_error && (ras_data[0] == error_pc + 'd8))?'b1:
							(bp_error && (ras_data[1] == error_pc + 'd8) && ras_pointer > 1)?'b1:
							(bp_error && (ras_data[2] == error_pc + 'd8) && ras_pointer > 2)?'b1:
							(bp_error && (ras_data[3] == error_pc + 'd8) && ras_pointer > 3)?'b1:
							(bp_error && (ras_data[4] == error_pc + 'd8) && ras_pointer > 4)?'b1:
							(bp_error && (ras_data[5] == error_pc + 'd8) && ras_pointer > 5)?'b1:
							(bp_error && (ras_data[6] == error_pc + 'd8) && ras_pointer > 6)?'b1:
							(bp_error && (ras_data[7] == error_pc + 'd8))?'b1:'b0;
	...
	always @(posedge clk) begin
		if (!rst_) begin
			...
			case({ras_read_en, ras_write_en})
				...
				2'b01: begin
					ras_pointer 	<= (!repeat_en)?(ras_pointer + 3'b1) : ras_pointer;
					repeat_counter  <= (!repeat_en)?(repeat_counter + 8'b1) : repeat_counter;
					ras_data[ras_pointer] <= (!repeat_en)?ras_write_data : ras_data[ras_pointer];
					last_pc <= call_pc;
				end
				2'b10: begin
					ras_pointer 	<= (repeat_taken)? ras_pointer : (ras_pointer - 3'b1);
					repeat_counter  <= (repeat_taken)? (repeat_counter - 8'b1) : repeat_counter;
				end
				2'b11: begin
					//keep the data
				end
